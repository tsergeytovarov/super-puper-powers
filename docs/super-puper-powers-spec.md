...
4. Pipeline опирается на память: необязательный журнал `docs/spp/pipeline-state.md` фиксирует, что уже сделано и какие решения приняты; сессия читает его для контекста, но порядок фаз он не навязывает.
...
- Каждый фазовый скилл: (a) если `pipeline-state.md` ведётся — читает его как память перед работой; (b) записывает артефакт в `docs/spp/`; (c) если журнал ведётся — обновляет его после гейта; (d) явно называет следующий скилл. Ведение журнала не обязательно для работы скилла — обязателен только артефакт.
...
Путь: `docs/spp/pipeline-state.md` (в репозитории продукта, не плагина). Необязательный: это память между чатами и лог решений, не гейт. Он не блокирует ни один скилл и не решает, что можно вызвать — любой скилл вызывается напрямую, если контекст того требует (диспетчеринг — как в upstream `using-superpowers`, §5.0). Создаётся в idea-intake, если pipeline стартует с нуля; при входе с середины (§5.0) агент создаёт его сам как журнал уже принятых решений. Формат — markdown с YAML-блоком:
...
current_phase: 0..9 | done    # журнальная запись «где мы», не переключатель доступа
...
Все поля — бухгалтерия для памяти и для скиллов, которые явно её читают (`plan-writing` → `pipeline_profile`, `cross-spec-review` → `subproject_order`, `deploy-strategy`/`post-release` → `deploy_status`). Ни одно поле не гейтит вызов скилла: гейты фазы 6, которые реально блокируют (или требуют явного «да»), читают артефакты на диске, а не эти поля — см. §5.0.

- **`current_phase`** — журнальная отметка последнего активного/завершённого этапа для памяти и для фразы «Pipeline на фазе N, продолжаю с …» (§5.0); терминальное значение `current_phase: done` пишется после фазы 9 (§5.13). Это запись для чтения человеком и агентом между чатами, не блокировка следующей фазы.
- **`pipeline_profile`**: `plan-writing` оценивает размер плана; если план ≤ 3 задач и суммарно ≤ ~150 строк — рекомендует `lite`. В `lite` фаза 6 использует облегчённый путь (один имплементер на план + одно финальное whole-branch ревью, без per-task fresh-субагента и per-task review); spec-review/plan-review сохраняются. Иначе `full`. Протокол lite описан в оркестраторе (§5.0), не в vendored SDD.
- **`deploy_status`**: `deploy-strategy` (§5.12) закрывает гейт в одном из режимов — `executed` (реальный деплой + прод-смоук с evidence) или `deferred` (стратегия выбрана, runbook готов, деплой отложен, live-evidence не требуется). При `deferred` `post-release` (§5.13) не считает продукт живым и пишет памятку в режиме «когда задеплоишь».

Плюс секции: `## Decisions log` (дата, фаза, решение, кем принято) и `## Artifacts` (ссылки на файлы фаз). Каждый гейт добавляет запись в Decisions log — это тоже память, не enforcement: пропущенный или неполный лог не блокирует работу, но обедняет резюмируемость.

**Как это используется на практике:**

1. Скилл фазы N при начале и завершении работы обновляет `current_phase` и, по итогу гейта, добавляет запись в Decisions log — это бухгалтерия для следующей сессии, не переключение доступа.
2. Оркестратор при старте сессии читает журнал как контекст (§5.0): называет, где проект остановился, и предлагает продолжить — пользователь решает сам, продолжать ли именно с той фазы или вызвать что-то другое напрямую.
3. `current_phase: done` (фаза 9 завершена, §5.13) — терминальная запись для памяти. Она не «запирает» pipeline: цикл post-release (§5.13) явно заводит его обратно на фазу 0 или 2 по фидбеку.
4. Единственные реально блокирующие/предупреждающие проверки во всём pipeline — фазовые гейты фазы 6, и они читают артефакты на диске (`docs/spp/06-data-boundaries.md`, `docs/spp/06-pre-show-audit.md`, `docs/spp/06-acceptance-demo.md`), а не поля этого журнала — см. §5.0.
...
- Таблица фаз 0–9 → скилл → артефакт → гейт (карта pipeline) — **рекомендованный маршрут, не enforced**: любой скилл вызывается напрямую вне зависимости от позиции в таблице; диспетчер маршрутизирует по контексту запроса, как upstream `using-superpowers`, а не проводит пользователя по фазам силой.
- При старте сессии: если существует `docs/spp/pipeline-state.md` — прочитать как **память** (что это за проект, что уже сделано, какие решения приняты), объявить «Pipeline на фазе N, продолжаю с …» и дать пользователю выбрать; журнал не используется, чтобы принудительно «продолжить фазу N». Если журнала нет и пользователь описывает продуктовую идею — предложить запустить pipeline с фазы 0.
- Никакого кросс-фазового hard-gate «фаза N+1 не начинается, пока гейт фазы N не approved»: это была бы блокировка по журналу, а журнал ничего не гейтит (§4). Фаза 6 — исключение: у неё есть собственные проверки, но они читают артефакты на диске, а не state, см. ниже.
- **Оркестратор — владелец гейтов фазы 6** (единственная фаза, чей рабочий скилл — vendored SDD — про эти проверки не знает):
  - после того как SDD завершил финальное whole-branch ревью и до **acceptance-демо**, оркестратор прогоняет два чекпоинта — `data-boundaries`, затем `pre-show-audit` — каждый пишет свой артефакт (`docs/spp/06-data-boundaries.md`, `docs/spp/06-pre-show-audit.md`); демо не стартует, пока оба файла не появились на диске — оркестратор проверяет это, открывая файлы, а не по памяти и не по журналу — `<HARD-GATE>`;
  - затем оркестратор проводит **acceptance-демо** (§5.10) и пишет артефакт `docs/spp/06-acceptance-demo.md`: перечень must-сценариев из MVP-scope, для каждого — как продемонстрирован и результат;
  - переход в `finishing-a-development-branch` (в т.ч. изнутри vendored SDD, которая иначе ведёт туда напрямую) — **safety-гейт, не hard gate**: оркестратор машинно проверяет на диске, что `docs/spp/06-acceptance-demo.md` существует и демо записано как принятое. Если нет — не блокирует молча и не продолжает автоматически, а прямо предупреждает пользователя, что демо не пройдено и релиз/мёрдж рискован, и требует явного «да, всё равно продолжаем» перед тем как продолжить;
  - после успешного демо (или явного «да» на предупреждении) оркестратор передаёт управление в `release-fixation` (фаза 7), которая и вызывает finishing (§5.11).
- Разрешён вход с середины: пользователь может явно сказать «у меня уже есть MVP-scope» — тогда оркестратор (если ведёт журнал) создаёт журнал, пишет номера пропущенных фаз в `phases_skipped`, фиксирует решение в Decisions log и маршрутизирует сразу к скиллу указанной фазы; ведение журнала не обязательно для входа с середины.
...
- `commands/spp.md` — явная команда `/spp`, дублирующая вход в оркестратор: если журнал есть — читает его как память и предлагает продолжить с последней записанной фазы (пользователь выбирает); если журнала нет — предлагает начать с фазы 0. Аргументов в v0.1 нет. Frontmatter команды: `description` одной строкой.

### 5.1. `idea-intake` (фаза 0)

- **Триггер:** пользователь описывает продуктовую идею; журнала ещё нет.
...
- **Гейт:** решение go / pivot / stop. При pivot — возврат в `idea-intake` с правками brief: журнал (если ведётся) записывает `current_phase: 0` и решение pivot в Decisions log. При stop — pipeline корректно завершается: журнал (если ведётся) фиксирует остановку и причину в Decisions log; это **успешный** исход, скилл обязан это проговорить, а не механическая блокировка — ничто не мешает вернуться к идее позже вне журнала.
...
- **Выход:** findings + рекомендованный порядок реализации под-проектов → поле `subproject_order` в журнале (потребители: `plan-writing` — порядок планов, оркестратор фазы 6 — порядок исполнения).
...
2. Сквозной тест на игрушечной идее (например, «tg-бот-напоминалка»): pipeline проходит фазы 0–5 с корректными артефактами в `docs/spp/` и записями в журнале; каждый гейт сформулирован на языке продукта (проверка: в тексте гейта нет слов diff/архитектура/рефакторинг и просьб читать код).
3. Resumability: прервать сессию на фазе 2, начать новую — оркестратор читает журнал как память, называет фазу 2 и уже принятые решения без повторных вопросов, и даёт пользователю продолжить с неё (выбор пользователя, не автопродолжение).
...
7. Discovery-стоп: на идее с очевидным юр-убийцей quick-режим находит стоп и корректно завершает pipeline как успех (журнал, если ведётся, записывает остановку и причину в Decisions log).
