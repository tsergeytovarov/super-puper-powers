...
8. **Убрать upstream-шаг «present the design» пользователю** (§5.5): секция «Presenting the design» и её чеклист-пункт велят показывать пользователю architecture / data flow / error handling с посекционным одобрением — это противоречит правке 3 и §1.1. Технические секции агент фиксирует в спеке; пользователю идёт только продуктовое резюме (п.6). Переписать «Cover: architecture, components, data flow, error handling, testing» на продуктовое поведение/сценарии/UX; убрать из чеклиста и Process Flow-графа узел «user approves design sections».
9. **Вырезать cross-plugin ссылку** (§5.5): `elements-of-style:writing-clearly-and-concisely` (строка Documentation) — сторонний плагин, в SPP не входит, у пользователя не резолвится. Заменить прозой «Write clearly and concisely» без имени скилла.

- [ ] **Step 3: Проверить**: `grep -niE 'visual[- ]companion|superpowers/specs|elements-of-style' skills/spec-writing/SKILL.md | grep -v 'Modifications:'; echo exit=$?` → пусто, exit=1. `grep -c 'spec-review' skills/spec-writing/SKILL.md` → ≥1. `grep -nE '\bwriting-plans' skills/spec-writing/SKILL.md | grep -v 'plan-writing'; echo exit=$?` → пусто, exit=1. Плюс глазами: секция показа дизайна не выносит architecture/data flow на одобрение пользователя.
