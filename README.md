...
(`using-superpowers` и `using-super-puper-powers`). Отсюда два следствия, и они разной
тяжести.

Риск «сработает не тот оркестратор» SPP снимает сам. Его хук на старте читает
`enabledPlugins` в настройках Claude Code и проверяет, активен ли рядом upstream-superpowers.
Если да — дописывает в свой инжект строку приоритета: pipeline ведёт SPP-оркестратор,
upstream-версию для фаз использовать не нужно. Ручных действий для этого не требуется, всё
происходит на старте сессии.

Раздувание постоянного контекста остаётся. Оба хука всё равно инжектят по большому блоку, и
чужой хук SPP отменить не может. Если постоянный контекст важен — отключите superpowers на
...
Имя маркетплейса подставьте своё (посмотреть можно через `/plugin marketplace list`). После
работы включите обратно, если пользуетесь им для другого.
